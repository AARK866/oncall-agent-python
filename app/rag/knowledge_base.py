...
class KnowledgeBase:
    ...
    def __init__(self, documents: list[RawDocument], chunks: list[DocumentChunk]) -> None:
        self.documents = documents
        self.chunks = chunks
        self._documents_by_id = {document.doc_id: document for document in documents}
        self._retriever = LocalKnowledgeBase(chunks=chunks)
    ...
    def search(
        ...
        return self._retriever.search(
            query=_build_query(query=query, keywords=keywords),
            top_k=top_k,
