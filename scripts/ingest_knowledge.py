...
async def main() -> int:
    ...
    incremental = result.metadata.get("incremental", {})
    print(f"- indexing_mode: {incremental.get('mode', 'full')}")
    if incremental.get("enabled"):
        print(f"- indexed_documents: {incremental['indexed_documents']}")
        print(f"- unchanged_documents: {incremental['unchanged_documents']}")
        print(f"- deleted_documents: {incremental['deleted_documents']}")
        print(f"- deleted_chunks: {incremental['deleted_chunks']}")
    print("- document_ids:")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--full-rebuild",
        action="store_true",
        help="Reindex every current document and remove stale manifest entries.",
    )
